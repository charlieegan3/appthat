...
Channel.delete_all
Channel.create(title: 'Social', tags: %w(feed muted blocked fb ig twitter snapchat stories tweets social instagram post tinder messages follow blocks friends tweet facebook insta unfollow posts facetime contacts relationship share friend talking sender unfollows block))
...
Channel.create(title: 'Geo', tags: %w(nearest locate uber local drive walk places area miles directions roads locations location weather nearby delivers))
Channel.create(title: 'Food', tags: %w(chocolate food pizza bed meal coffee restaurants sandwich oven snacks bar))
...
Channel.create(title: 'Internet & Data', tags: %w(upload data online download downloaded downloading wifi data))
Channel.create(title: 'Tech', tags: %w(technology tablet laptop screen tech))
Channel.create(title: 'Mobile', tags: %w(android iphone ios mobile phone))
Channel.create(title: 'Intelligent Tool', tags: %w(rates guess tracks caches scans calculate edits build rate answer search suggestions learn track))
...
Channel.create(title: 'Productivity', tags: %w(productive todo organize))
